...
const removeMarker = function (id, mapParam = undefined) {
    let map = this;
    ...
    if (id) {
        ...
        if (layer) {
            ...
            map.removeLayer(id);
            map.removeSource(id);
            delete markers[markers.indexOf(id)];
        } else {
...
const addMarkerArray = function (payload = {}) {
    ...
    const markerArray = payload.marker ? payload.marker.map((val, index) => {
        if (!val.lng || !val.lat) {
...
const addMarkerImageArray = function (info = {}) {
    ...
    const markerArray = places ? places.map((val, index) => {
        if (!val.lng || !val.lat) {
